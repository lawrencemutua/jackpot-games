...
import com.jackpotgameslawi.databinding.ActivitySportpesaMiniBinding

class SportpesaMiniActivity : AppCompatActivity() {
    private lateinit var binding: ActivitySportpesaMiniBinding
    private lateinit var interstitialAd: InterstitialAd
    private lateinit var adView: AdView
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivitySportpesaMiniBinding.inflate(layoutInflater)
        setContentView(binding.root)

        val actionBar = supportActionBar
        actionBar!!.title = "Sportpesa Mini JP"
        actionBar.setDisplayHomeAsUpEnabled(true)

        binding.recyclerSportpesa.apply {
            layoutManager = LinearLayoutManager(this@SportpesaMiniActivity)
        }
        pickmini()
    }

    private fun pickmini() {
        FirebaseFirestore.getInstance().collection("sportpesamini")
            .get()
