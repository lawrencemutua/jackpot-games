...
import com.jackpotgameslawi.databinding.ActivityFoot10Binding

class Foot10Activity : AppCompatActivity() {
    private lateinit var binding: ActivityFoot10Binding
    private lateinit var adView: AdView
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityFoot10Binding.inflate(layoutInflater)
        setContentView(binding.root)

        val actionBar = supportActionBar
        actionBar!!.title = "10+ ODDS"
        actionBar.setDisplayHomeAsUpEnabled(true)

        binding.recyclerSafepicks.apply {
            layoutManager = LinearLayoutManager(this@Foot10Activity)
        }
    ...
    private fun pick10() {
        FirebaseFirestore.getInstance().collection("10+ODDS")
            .get()
