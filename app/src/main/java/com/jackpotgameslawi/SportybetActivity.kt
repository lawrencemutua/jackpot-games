...
import com.jackpotgameslawi.databinding.ActivitySportybetBinding

class SportybetActivity : AppCompatActivity() {
    private lateinit var binding: ActivitySportybetBinding
    private lateinit var adView: AdView
    private lateinit var interstitialAd: InterstitialAd
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivitySportybetBinding.inflate(layoutInflater)
        setContentView(binding.root)

        val actionBar = supportActionBar
        actionBar!!.title = "Sportybet JP"
        actionBar.setDisplayHomeAsUpEnabled(true)

        binding.recyclerSportpesa.apply {
            layoutManager = LinearLayoutManager(this@SportybetActivity)
        }
        fetchsporty()
    }

    private fun fetchsporty() {
        FirebaseFirestore.getInstance().collection("sportybet")
            .get()
