...
import com.jackpotgameslawi.databinding.ActivityBetikaMidBinding

class BetikaMidActivity : AppCompatActivity() {
    private lateinit var binding: ActivityBetikaMidBinding
    private lateinit var adView: AdView
    private lateinit var interstitialAd: InterstitialAd
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityBetikaMidBinding.inflate(layoutInflater)
        setContentView(binding.root)

        val actionBar = supportActionBar
        actionBar!!.title = "Betika Midweek JP"
        actionBar.setDisplayHomeAsUpEnabled(true)

        binding.recyclerSportpesa.apply {
            layoutManager = LinearLayoutManager(this@BetikaMidActivity)
        }

        pickmidweek()
    }

    private fun pickmidweek() {
        FirebaseFirestore.getInstance().collection("betika-midweek")
            .get()
