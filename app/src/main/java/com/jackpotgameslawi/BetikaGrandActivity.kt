...
import com.jackpotgameslawi.databinding.ActivityBetikaGrandBinding

class BetikaGrandActivity : AppCompatActivity() {
    private lateinit var binding: ActivityBetikaGrandBinding
    private lateinit var adView: AdView
    private lateinit var interstitialAd: InterstitialAd
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityBetikaGrandBinding.inflate(layoutInflater)
        setContentView(binding.root)

        val actionBar = supportActionBar
        actionBar!!.title = "Betika Grand JP"
        actionBar.setDisplayHomeAsUpEnabled(true)

        binding.recyclerSportpesa.apply {
            layoutManager = LinearLayoutManager(this@BetikaGrandActivity)
        }
        pickgrand()
    }

    private fun pickgrand() {
        FirebaseFirestore.getInstance().collection("betika-grand")
            .get()
